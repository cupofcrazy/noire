...
import { IconQuickSearch, IconCrossSmall, IconBarsTwo } from "central-icons"
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 md:hidden">
      <PopoverPrimitive.Root open={open} onOpenChange={setOpen}>
        <div className="flex items-center gap-1 rounded-full border border-border bg-background/80 p-1 shadow-lg backdrop-blur-xl">
          <ThemeToggle />
          ...
          >
            <AnimatePresence mode="popLayout">
              ...
              >
                {open ? (
                  <IconCrossSmall className="size-5" />
                ) : (
        ...
        <PopoverPrimitive.Portal>
          ...
          >
            <PopoverPrimitive.Popup
              className={cn(
                "w-72 max-h-[min(24rem,calc(100svh-6rem))] overflow-y-auto rounded-xl border border-border bg-popover shadow-lg outline-none",
                ...
                {filtered.map((demo) => {
                  ...
                  return (
                    <Link
                      ...
                      href={href}
                      className={cn(
                        "text-foreground/80 rounded-lg h-8 px-3 flex items-center gap-2 text-sm font-medium transition-colors",
                        isActive
